class Solution {
    ...
    public int openLock(String[] deadends, String target) {
        ...
        while(visiting.size()>0){
            String str = visiting.poll();
            byte [] s = str.getBytes();
            int dist = visited.get(str);

            if(str.compareTo(target)==0) return dist;

            for(int i=0; i<4; i++){
                int d = s[i]-'0';
                s[i] = (byte)((d+1)%10 + '0');
                String temp = new String(s);
                if(!dead.contains(temp) && !visited.containsKey(temp)) {
                    visited.put(temp, dist+1);
                    visiting.offer(temp);
...
//case 27/48: ["0000"] "8888" 一開始就封殺 "0000" 要跳掉
//case 30/48: 不能用 PriorityQueue,要用LinkedList

# 這題是 Hard：依序在 m*n 裡的 positions[i] 放陸地
# 依序回傳「當時有幾個相連的島」
# 重點是，「當兩個島」因某「增加某塊陸地」而相連時，要結合
# 所以與 groups union / union find 有關
class Solution:
    def numIslands2(self, m: int, n: int, positions: List[List[int]]) -> List[int]:
        ans = []
        count = 0  # 目前不同的 group 數量
        groupN = 0  # 給 group[i] 用了幾個點
        group = [-1] * (m * n)  # 最多有這麼多個格子，這麼多種 group, 待合併
        table = [[-1 for _ in range(n)] for _ in range(m)]
        dI = [0, 1, 0, -1]  # 右、下、左、上
        dJ = [1, 0, -1, 0]
        # table[i][j]==-1 表示還沒有設定 group
        for i, j in positions:  # 逐個「增加陸地」
            if table[i][j] != -1:  # 竟然已經有處理過了，直接避開
                ans.append(count)
                continue

            now = table[i][j] = group[groupN] = groupN
            groupN += 1
            count += 1
            # 接下來「上下左右」4個方向，看鄰居能否合併
            for k in range(4):
                ii, jj = i + dI[k], j + dJ[k]  # 往4個方法測

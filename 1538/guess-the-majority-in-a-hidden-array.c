...
int guessMajority(ArrayReader* reader) {
    ...
    int N = length(reader); //array lengh of a[i]
    bool same[N]; 
    ...
    int Q10 = query(reader, 0,  2,3,4);
    int Q11 = query(reader,   1,2,3,4);
    same[1] = (Q10==Q11); //maintain 2,3,4 => find 1

    int Q20 = query(reader, 0,1,  3,4);
    int Q22 = query(reader,   1,2,3,4);
    same[2] = (Q20==Q22); //maintain 1,3,4 => find 2

    int Q30 = query(reader, 0,1,2,  4); 
    int Q33 = query(reader,   1,2,3,4);
    ...
    int Q0 = query(reader, 0, 1,2,3); 
    for(int i=4; i<N; i++){
        same[i] = (query(reader, 1,2,3, i) == Q0);
    }

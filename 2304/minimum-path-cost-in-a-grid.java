class Solution {
    public int minPathCost(int[][] grid, int[][] moveCost) {
        ...
        for(int i=0; i<M-1; i++){
            for(int j=0; j<N; j++){
                //int now = grid[i][j];
                //int [] edge = moveCost[now];
                table[i+1][j] = Integer.MAX_VALUE;;//table[i][j]+edge[j];
            }
            for(int j=0; j<N; j++){
                int now = grid[i][j];
                int [] edge = moveCost[now];
                for(int k=0; k<N; k++){
                    table[i+1][k] = min(table[i+1][k], table[i][j] + edge[k]);
                }
            }
            for(int j=0; j<N; j++){//這個迴圈不能和前面迴圈合在一起，不然會出錯
        ...
        int ans = table[M-1][0];
        for(int j=0; j<N; j++){
            if(table[M-1][j]<ans) ans = table[M-1][j];
        }
        return ans;
    }
    int min(int a, int b) {
...
}//case3: [[28,35,29,5,13,17,18,23,14],[30,15,12,27,2,26,25,19,7],[1,16,34,21,9,3,20,24,8],[4,33,22,11,31,0,6,10,32]]
